...
namespace BarebonesVR
{
	...
	extern "C" {
		...
		bool SKSEPlugin_Query(const SKSEInterface* skse, PluginInfo* info) {	// Called by SKSE to learn about this plugin and check that it's safe to load it
			gLog.OpenRelative(CSIDL_MYDOCUMENTS, "\\My Games\\Skyrim VR\\SKSE\\2hWeaponsUnlocked.log");
		...
		void OnSKSEMessage(SKSEMessagingInterface::Message* msg)
		{
			if (msg)
			{
				...
				else if (msg->type == SKSEMessagingInterface::kMessage_PostPostLoad)
				{
					...
					BarebonesVR::RegisterSpellWheelTwoHandLogCallbacks(g_pluginHandle, g_messaging);
